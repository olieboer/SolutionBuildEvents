...
namespace SolutionBuildEvents
{
    ...
    internal sealed class EnableSolutionBuildEventsCommand
    {
        ...
        public async Task InitializeAsync(AsyncPackage package)
        {
            ...
            _dte.Events.BuildEvents.OnBuildBegin += async (scope, action) => HandleBuildEventAsync(action, true);
            _dte.Events.BuildEvents.OnBuildDone += async (scope, action) => HandleBuildEventAsync(action, false);
        }
        ...
        private async Task HandleBuildEventAsync(vsBuildAction action, bool isPreBuild)
        {
            if (!File.Exists(_configFilePath))
            {
                return;
